...
void initComponents(){
  ...
  attachInterrupt(digitalPinToInterrupt(buttonIntPin), clickCounter, CHANGE);
...
void resetGame(){
  ...
  if(playerScore > 0){
    avgReactionTime = (playTime * 100.0) / playerScore;
  } else {
    avgReactionTime = 0;
  }
...
volatile int lastTimeInterrupted = 0;
void clickCounter(){
  if(playerTurn && ((count_hundred_ms > lastTimeInterrupted+5) || (lastTimeInterrupted==0))){
    ...
    elapsedTurnTime = 0; //start the player's turn time
        ...
        while(elapsedTurnTime <= moleTime){
          ...
          if(digitalRead(holeButtons[randLEDSelector]) == LOW && !hitSuccess){
            ...
            Serial.println(moleTime * 100); //That * 100 is important because the Arduino stores interval in units of 100 ms, while your Python label says ms.

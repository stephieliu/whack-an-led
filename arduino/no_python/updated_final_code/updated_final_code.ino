...
void initGame(){
  Serial.println("Setting up a new game...");
...
void initTime(){
  Serial.println("Resetting time variables...");
...
void resetGame(){
  Serial.println("Ending the game...");

  //set status led RED
  int currentTime = count_hundred_ms;
  while(count_hundred_ms < currentTime + 1){ //keep the led on for 100ms
  ...
  Serial.print("You clicked a total of ");
  ...
  Serial.print("Your average reaction time in ms was: ");
...
void loop() {
  ...
  if(endGame){
    Serial.println("Ending the current game and starting a new one.");
  ...
  if((digitalRead(startButtonPin) == LOW) && !gameOn){
    //start the new game
    Serial.println("Beginning a new game...");
          ...
          if(digitalRead(holeButtons[randLEDSelector]) == LOW && !hitSuccess){
            ...
            if(moleTime > LOW_MOLE_THRESHOLD){
              moleTime -= DECREMENT_TIME;
           	...
           	Serial.println("Current time interval (ms) is set to: ");
            Serial.println(moleTime);
            break;
          }
//          Serial.println(numPlayerClicks);
        }
        //catch if the player failed to hit the mole this time
        if(!hitSuccess){
          //print the fail statement to the serial monitor
        	Serial.println("Failed to hit the mole...");
        ...
        Serial.println("Ended player's turn...");
